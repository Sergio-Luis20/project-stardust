 ...
 * 
 * <p>Not every class annotated with {@link Entity} must implement
 ...
 * of the entity in database (normally the primary key in relational model).</p>
 * 
 * <p>This interface extends {@link Serializable}, so implementations
 * must properly follow its contract and must deal with transient attributes
 * and custom serialization when necessary.</p>
 * 
 * <p>If this class is annotated with {@link Entity}, then the id returned
 * by {@link #getEntityId()} must be attribute annotated with {@link Id}.</p>
 * 
 * <p>The implementation class should have a public constructor with no
...
public interface StardustEntity<T> extends Serializable {
     ...
     * in relation model).
     * 
     * <p>It is recommended this id to be a primitive type, a wrapper type,
     ...
     * 
     * <p>If you are in the last recommended case, then it is also recommended the
